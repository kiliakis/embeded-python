...
def matched_from_line_density(beam_beta, beam_energy, beam_charge,
                              beam_n_macroparticles, beam_dt, beam_dE,
                              eta_0, t_rev_0, potential_well_array,
                              time_coord_array,
    ...
    *Function to generate a beam by inputing the line density. The distribution
    ...
    eom_factor_potential = np.sign(
        slippage_factor) * Beam.charge / (FullRingAndRF.RingAndRFSection_list[0].t_rev[0])

    # Generate potential well
    n_points_potential = int(1e4)
    # TODO this must be done before calling this function
    ...
    if extraVoltageDict is not None:
        ...
        extra_potential_input = - eom_factor_potential * \
            np.insert(cumtrapz(extra_voltage_input, dx=extra_voltage_time_input[1] -extra_voltage_time_input[0]), 0, 0)
        extra_potential = np.interp(
            time_coord_array, extra_voltage_time_input, extra_potential_input)
    ...
    if line_density_options['type'] is not 'user_input':
        ...
        time_line_den = np.linspace(
            time_coord_array[0], time_coord_array[-1], n_points_line_den)
        line_den_resolution = time_line_den[1] - time_line_den[0]

        # Normalizing the line density
        line_density = line_density_function(time_line_den, line_density_options['type'],
                                             line_density_options['bunch_length'],
                                             exponent=line_density_options['exponent'],
                                             bunch_position=(time_coord_array[0]+time_coord_array[-1])/2)

        line_density = line_density - np.min(line_density)
        line_density = line_density / \
            np.sum(line_density) * Beam.n_macroparticles
        ...
        raise RuntimeError(
            'The input for the matched_from_line_density function was not recognized')
    ...
    #     induced_voltage_object.slices.edges = np.linspace(induced_voltage_object.slices.bin_centers[0]-(induced_voltage_object.slices.bin_centers[1]-induced_voltage_object.slices.bin_centers[
    #                                                       0])/2, induced_voltage_object.slices.bin_centers[-1]+(induced_voltage_object.slices.bin_centers[1]-induced_voltage_object.slices.bin_centers[0])/2, len(induced_voltage_object.slices.bin_centers)+1)
    ...
    #     induced_voltage = induced_voltage_object.induced_voltage_sum(
    #         Beam, length=induced_voltage_length)
    #     time_induced_voltage = np.linspace(time_line_den[0], time_line_den[
    # 0] + (induced_voltage_length - 1) * line_den_resolution,
    # induced_voltage_length)

    #     # Calculating the induced potential
    #     induced_potential = - eom_factor_potential * \
    #         np.insert(cumtrapz(
    # induced_voltage, dx=time_induced_voltage[1] - time_induced_voltage[0]),
    # 0, 0)
    ...
    for i in range(0, n_iterations):
        ...
        total_potential = potential_well_array + \
            induced_potential_final + extra_potential

        # Process the potential well in order to take a frame around the
        # separatrix
        ...
        minmax_positions_profile, minmax_values_profile = minmax_location(
            time_line_den[line_density != 0], line_density[line_density != 0])
        ...
        if n_maxima_profile > 1:
            ...
            max_profile_pos = minmax_positions_profile[1][
                np.where(minmax_values_profile[1] == np.max(minmax_values_profile[1]))]
        else:
            max_profile_pos = minmax_positions_profile[1]
        if n_minima_potential > 1:
            ...
            min_potential_pos = minmax_positions_potential[0][
                np.where(minmax_values_potential[0] == np.min(minmax_values_potential[0]))]
        else:
            min_potential_pos = minmax_positions_potential[0]

        # Moving the bunch (not for the last iteration if intensity effects are
        # present)
        ...
        if i != n_iterations - 1:
            time_line_den = time_line_den - \
                (max_profile_pos - min_potential_pos)
            time_induced_voltage = np.linspace(time_line_den[0], time_line_den[
                                               0] + (induced_voltage_length - 1) * line_den_resolution, induced_voltage_length)
    ...
    for abel_index in range(0, abel_both_step):
        ...
        if half_option is 'first':
            half_indexes = np.where(
                (time_line_den >= time_line_den[0]) * (time_line_den <= max_profile_pos))
        if half_option is 'second':
            half_indexes = np.where(
                (time_line_den >= max_profile_pos) * (time_line_den <= time_line_den[-1]))
        ...
        potential_half = np.interp(
            time_coord_half, time_coord_sep, potential_well_sep)
        potential_half = potential_half - np.min(potential_half)

        # Derivative of the line density
        line_den_diff = np.diff(
            line_den_half) / (time_coord_half[1] - time_coord_half[0])

        time_line_den_diff = time_coord_half[
            :-1] + (time_coord_half[1] - time_coord_half[0]) / 2
        line_den_diff = np.interp(
            time_coord_half, time_line_den_diff, line_den_diff, left=0, right=0)

        # Interpolating the line density derivative and potential well for Abel
        # transform
        time_abel = np.linspace(
            time_coord_half[0], time_coord_half[-1], n_points_abel)
        line_den_diff_abel = np.interp(
            time_abel, time_coord_half, line_den_diff)
        ...
        if (half_option is 'first') or (half_option is 'both' and abel_index == 0):
            for i in range(0, n_points_abel):
                integrand = line_den_diff_abel[
                    0:i+1] / np.sqrt(potential_abel[0:i+1] - potential_abel[i])
                ...
                density_function[i] = np.sqrt(
                    eom_factor_dE) / np.pi * np.trapz(integrand, dx=time_coord_half[1] - time_coord_half[0])
        ...
        if (half_option is 'second') or (half_option is 'both' and abel_index == 1):
            for i in range(0, n_points_abel):
                integrand = line_den_diff_abel[
                    i:] / np.sqrt(potential_abel[i:] - potential_abel[i])
                ...
                density_function[i] = - np.sqrt(eom_factor_dE) / np.pi * np.trapz(
                    integrand, dx=time_coord_half[1] - time_coord_half[0])
                hamiltonian_coord[i] = potential_abel[i]
    ...
    if half_option is 'both':
        ...
        density_function = (density_function_average[
                            :, 0] + np.interp(hamiltonian_coord, hamiltonian_average[:, 1], density_function_average[:, 1])) / 2
    ...
    density_grid = np.interp(
        hamiltonian_grid, hamiltonian_coord, density_function)
    ...
    indexes = np.random.choice(
        range(0, np.size(density_grid)), Beam.n_macroparticles, p=density_grid.flatten())

    Beam.dt = np.ascontiguousarray(time_grid.flatten()[indexes]+(np.random.rand(
        Beam.n_macroparticles) - 0.5)*(time_coord_for_grid[1]-time_coord_for_grid[0]))

    Beam.dE = np.ascontiguousarray(deltaE_grid.flatten()[
                                   indexes] + (np.random.rand(Beam.n_macroparticles) - 0.5) * (deltaE_for_grid[1]-deltaE_for_grid[0]))

    # if TotalInducedVoltage is not None:
    #     # Inputing new line density
    #     induced_voltage_object.slices.n_macroparticles = reconstructed_line_den * \
    ...
    # else:
    # return [hamiltonian_coord, density_function], [time_line_den,
    # line_density]
...
# while np.abs(distribution_options['bunch_length']-tau) >
# bunch_length_accuracy:
...
# print 'WARNING : The desired bunch length is too small to be generated
# accurately !'
